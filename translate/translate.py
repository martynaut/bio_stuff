    ...
    if start == True:
        ...
        for j in results:
            ...
            for i in range(0,len(j),3):
                    prot_sequences[len(prot_sequences)-1] = prot_sequences[len(prot_sequences)-1]+translation_code[j[i:i+3]]
                    if len(j[i+3+1:]) < 3:
                        break
        ...
        for j in frames:
            for i in range(j,len(sequence),3):
                prot_sequences[j] = prot_sequences[j]+translation_code[sequence[i:i+3]]
